        ...
        <div className="flex justify-center items-center flex-wrap h-full g-6 text-gray-800">
          ...
          <div className="md:w-8/12 lg:w-5/12 lg:ml-20 container bg-slate-300 px-4 py-6 rounded-xl ">
            <Card className='px-8 py-10'>
              <h1 className='font-bold text-4xl text-center my-4'>Bienvenido</h1>
              <p className='text-slate-100 mb-2'>Ingresa tus credenicales.</p>
              <form onSubmit={handleSubmit(onSubmit)}>
                <div className="mb-4">
                  <Input {...register("username", {required : true})} className='mx-auto' color='purple' variant='outlined' label='Usuario' size='lg'/>
                  {errors.username && <span className="text-red-400">Usuario requerido</span>}
                </div>
                <div className="mb-6">
                  <Input {...register("password", {required : true})} type="password" variant='outlined' color='purple' label='Contraseña' size='lg'/>
                  {errors.password && <span className="text-red-400">Contraseña requerida</span>}                
                </div>
                <div className='mx-auto w-max'>
                  <Button onClick={handleSubmit(onSubmit)} size='lg' color='purple'>Iniciar Sesion</Button>
                </div>
                <div className="mx-auto my-2">
                  <Fragment>
                    <Alert size="sm" className="" color="red" show={loginError} dismissible={{ onClose: () => setLoginError(false),}}>
